...
namespace AdventOfCode.Y2016 {
    public class Puzzle20 : ASolver {
        private List<BlockedRange> blocked;
        public Puzzle20(string input) : base(input) { Name = "Firewall Rules"; }

        public override void Setup() {
            List<string> items = Tools.GetLines(Input);
            blocked = new List<BlockedRange>();
            for (int i = 0; i < items.Count; i++) {
                string item = items[i];
                int index = item.IndexOf('-');
                blocked.Add(new BlockedRange() { Start = Tools.ParseUInt(item, 0, index), End = Tools.ParseUInt(item, index + 1) });
            }
            blocked.Sort();
        }
