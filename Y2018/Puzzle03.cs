...
namespace AdventOfCode.Y2018 {
    public class Puzzle03 : ASolver {
        ...
        public Puzzle03(string input) : base(input) { Name = "No Matter How You Slice It"; }
        ...
        private class Claim {
            ...
            public static implicit operator Claim(string value) {
                ...
                claim.ID = Tools.ParseInt(value.Substring(1, index1));

                int index2 = value.IndexOf(',', index1);
                claim.X = Tools.ParseInt(value.Substring(index1 + 3, index2 - index1 - 3));

                index1 = value.IndexOf(':', index2);
                claim.Y = Tools.ParseInt(value.Substring(index2 + 1, index1 - index2 - 1));

                index2 = value.IndexOf('x', index1);
                claim.W = Tools.ParseInt(value.Substring(index1 + 2, index2 - index1 - 2));
                claim.H = Tools.ParseInt(value.Substring(index2 + 1));
